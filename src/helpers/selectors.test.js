...
const state = {
  days: [
    { id: 1, name: "Monday", appointments: [1, 2, 3], interviewers: [1, 2] },
    { id: 2, name: "Tuesday", appointments: [4, 5], interviewers: [2, 3] },
    { id: 3, name: "Wednesday", appointments: [1, 5], interviewers: [2, 4] },
    { id: 4, name: "Thursday", appointments: [3, 4, 5], interviewers: [2, 5] },
    { id: 5, name: "Friday", appointments: [2, 3, 4], interviewers: [1, 2] }
  ],
  appointments: {
...
test("getInterviewersForDay returns an empty array when the day is not found", () => {
  const result = getAppointmentsForDay(state, "Sundawy");
  expect(result.length).toEqual(0);
});
